h2. ScalaCollider-Swing ==<small>-- version 0.12 (built 05-may-10) --</small>==
...
ScalaCollider-Swing currently compiles against the Scala 2.8.0.RC1 and requires Java 1.6. The repository contains a project file for Intellij IDEA 9 CE, and project files for sbt. Targets for sbt:
...
Where @standalone@ creates a fully self-contained jar (this requires that you have shell programs @unzip@ and @jar@ available), and @package-app@ updates the Mac OS X application bundle.
...
Note that due to an incompatibility between Scala 2.8.0-RC1 and sbt 0.7.3, it is not possible to run using @sbt run@. You can use the application bundle instead on OS X:
